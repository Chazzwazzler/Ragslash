...
public class DragTool : MonoBehaviour
{
    ...
    void Update()
    {
        if(!InputHelper.MouseOverUIObject){
            if(ToolController.currentlySelectedTool == ToolController.Tools.dragTool){
                ...
                if(Input.GetKeyUp(Keybinds.usePrimaryToolFunction) && objectUnderMouse.GetComponent<Draggable>() != null){
                    objectUnderMouse.GetComponent<Rigidbody2D>().bodyType = objectUnderMouse.GetComponent<OriginalObjectPropertiesManager>().currentRigidbodyState;
                    draggingMouse = false;
